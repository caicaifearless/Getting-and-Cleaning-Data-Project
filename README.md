...
1. Load to R for features being measured in experiment from file ***features.txt***.

2. Load complete data for train sets:
    a. Firstly loads the measurements from ***X_train.txt*** as a data frame  
    b. For these data frame column names are changed to be features loaded on the step 1. (**STEP 4**: *Appropriately label the data set with descriptive variable names* of Course Project  
  ...
  Similar steps are made for test dataset and finally 2 data frames are merged together by rows to form data frame with complete data (**STEP 1**: *Merge the training and the test sets to create one data set* of assignment)
...
4. To provide descriptive values for activity labels, change the original acativity columns to factor that variable with levels mentioned in file activity_labels.txt (**STEP 3**: *Use descriptive activity names to name the activities in the data set* of assignment)
